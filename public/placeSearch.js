...
function displayInfowindow(marker, title, id, name) {
    ...
    let content = `<div class='infowindow' >` + title + '</div>' +
        '<a style="margin:5px; color: blue;" href="' + kakaoMapURL + '" target="_blank">자세히보기</a><br/>' +
...
async function selectPlace(){
    ...
    let staticMapContainer2  = document.getElementById('myPlace'), // 이미지 지도를 표시할 div
        staticMapOption = { 
    ...
    // 이미지 지도를 생성합니다
    let staticMap = new kakao.maps.StaticMap(staticMapContainer2, staticMapOption);
}